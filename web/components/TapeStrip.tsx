...
export function TapeStrip({ only }: { only?: readonly string[] }) {
  const tape = useTape();
  // a symbol with no ticker is one we cannot name, let alone price
  const list = (only ?? POOL).filter((t) => t !== "?");
      ...
      <div className="grid gap-2">
        {list.map((t) => {
          ...
          return (
            <div
              key={t}
              className="flex flex-wrap items-center gap-x-3 gap-y-1 rounded-lg bg-panel2 px-3 py-2.5"
            >
              <span
                className="h-2 w-2 shrink-0 rounded-full"
                style={{ background: allow === undefined ? "#3a4256" : allow ? "var(--good)" : "var(--hold)" }}
              />
              <span className="mono w-16 shrink-0 text-[13px]">{t}</span>
